...
Since the roughly 6000\,K photosphere \jdp{\sout{is incapable of} does not have sufficient thermal energy to} heat the corona to a million degrees, the energy has to come from somewhere else. 
\cck{(In what sense, i.e., under what assumptions? Gasoline and oxygen, both at room temperature, evidently heats the engine of my car to over $200^{\circ}$F. Cold water going through a turbine in Hoover dam, or cold air interacting with a windmill in Judith Gap, somehow manages to toast my bread.)} 
...
\jdp{The velocity at which waves travel along magnetic field lines, which occurs post reconnection, is known as the Alfv\'en speed. 
In areas of strong magnetic field the Alfv\'en velocity can be an order of magnitude faster than the local sound speed in the plasma \citep{Priest2014}, which is the fastest speed a wave can travel through fluid processes.}
...
\cck{(Explanation seems a bit muddled. Both wave speeds are relevant for the plasma, but which one is faster depends on $\beta$, which is the ratio of sound speed to Alfv\'en speed.)} \jdp{Yeah, I see your point.  Any better?}
...
Light emitted from the sun comes in a wide ($\approx3000$\, nm) range of wavelengths \cck{(Way wider than that. From Gamma rays to radio. When the range covers so many orders of magnitude, does it make sense to talk about a wavelength (or frequency) interval?)}, referred to as the solar spectrum, with it's peak emission being visible light ($\approx400-700$\, nm) \citep{JudgeBook}.
The solar spectrum is broadly peaked around visible light, but zooming in reveals many smaller peaks and dips at various wavelengths. \jdp{I might need a figure for this.} \cck{(Or even for a sample part of it. You can grab a very broad SSI spectrum from LISIRD (I think they have citation instructions))}
...
As temperature increases from the photosphere up through the rest of the solar atmosphere, the sun emits generally shorter wavelengths. \jdp{True in general?} \cck{(Doubtless you have noticed it doesn't work this way with the AIA channels. Higher energy transitions become accessible at higher temperatures, but the plot would be like a filled-in triangle; think of it as $hc/\lambda<\frac{1}{2}kT$. In other words, there is a (rough) lower limit on the wavelength that is accessible from a given temperature, but there is no upper limit on the wavelengths that are possible. For example, think of the classic coronal green line in eclipse images, which ultimately led to the discovery that the corona is way stinkin' hot (but not before someone proposed a new element, `coronium' as the explanation for this line!))}
...
\jdp{This last paragraph needs some work.  I need more concrete information and sources about the trade offs in spatial and spectral resolution.}
Compared to slit spectrographs, CTIS instruments compromise spatial and spectral resolution, as well as ease of data analysis, for increased field of view and a faster temporal cadence. \cck{I guess I would say that the trade-off is ambiguity, not resolution (spectral or spatial). What, after all, is the spatial or spectral resolution limit of a CTIS? I don't think there is any intrinsic limit, just practical limits like pixels and diffraction that would apply to any grating spectrograph.}
...
Also, since the transition region is very thin, we can observe these events with less plasma in front of, or behind, the region of interest. \cck{This reminds me that the notion of optically thin lines has not come up, so it isn't apparent at this stage why you would be concerned \emph{both} about material before and behind the events of interest.}
...
\cck{(Curious why you chose reverse chrono order.)} The first chapter explores data from the first flight of the ESIS sounding rocket launch. 
